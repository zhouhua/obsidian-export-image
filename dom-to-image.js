...
export function toSvgWithClonedDom(node, clone, options) {
  options = options || {};
  copyOptions(options);
  return Promise.resolve(clone)
    ...
    .then(applyOptions)
    .then(function (clone) {
      return makeSvgDataUri(clone, options.width * 2, options.height * 2);
    });
...
export function toBlobWithClonedDom(node, clone, options) {
  return drawWithClonedDom(node, clone, options || {}).then(function (canvas) {
    return util.canvasToBlob(canvas, options);
  });
}
...
export function drawWithClonedDom(domNode, clone, options) {
  return toSvgWithClonedDom(domNode, clone, options)
    ...
    .then(function (image) {
      ...
      canvas
        .getContext("2d")
        .drawImage(image, 0, 0, options.width * 2, options.height * 2);
      return canvas;
    });
  ...
  function toBlob(canvas, options) {
    return new Promise(function (resolve) {
      var binaryString = window.atob(
        canvas.toDataURL("image/jpeg", options.quality || 1.0).split(",")[1]
      );
